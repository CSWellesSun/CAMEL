...
if __name__ == "__main__":
    ...
    try:
        with open(
            os.path.join(args.model_path, "model.safetensors.index.json"), "r"
        ) as f:
            index_json = json.loads(f.read())
            head_path = index_json["weight_map"]["lm_head.weight"]
        with safe_open(
            os.path.join(args.model_path, head_path), framework="pt", device="cpu"
        ) as f:
        ...
        with open(
            os.path.join(args.model_path, "pytorch_model.bin.index.json"), "r"
        ) as f:
            index_json = json.loads(f.read())
            head_path = index_json["weight_map"]["lm_head.weight"]
        weights = torch.load(os.path.join(args.model_path, head_path))
