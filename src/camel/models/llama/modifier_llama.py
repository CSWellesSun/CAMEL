...
class Model(nn.Module):
    ...
    def forward(
        ...
        std=None,
        compression_past_key_values: Optional[List[torch.FloatTensor]] = None,
    ):
        ...
        speculation_attention_mask = self._prepare_decoder_attention_mask(
            attention_mask,
        ...
        compression_attention_mask = self._prepare_compression_decoder_attention_mask(
            attention_mask,
            (batch_size, seq_length),
            hidden_states,
            past_key_values_length,
        ...
        for idx, decoder_layer in enumerate(self.compression_layers):
            if output_hidden_states:
                all_hidden_states += (hidden_states,)

            compression_past_key_value = (
                compression_past_key_values[idx]
        ...
        if use_cache:
            return hidden_states, next_decoder_cache, compression_next_decoder_cache
